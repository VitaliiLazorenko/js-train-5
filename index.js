...
function modifyArray(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    // Перетворюємо кожен елемент в рядок за допомогою toString()
    array[i] = array[i].toString();
  }
  console.log(array.join()); // Виводимо масив перетворений в рядок
}
...
function printUppercase(array) {
  for (let i = 0; i < array.length; i++) {
    ...
    console.log(array[i].toUpperCase()); // Перетворюємо кожен елемент в верхній регістр та виводимо на консоль
  }
}
...
function findMinMax([firstElement, ...restElements]) {
  ...
  for (let i = 0; i < restElements.length; i++) {
    ...
    if (restElements[i] < min) {
      ...
      min = restElements[i]; // Якщо так, оновлюємо мінімальне значення
    }
    if (restElements[i] > max) {
      // Перевіряємо, чи поточний елемент більший за максимальне значення
      max = restElements[i]; // Якщо так, оновлюємо максимальне значення
    }
  }
...
function printStrings(array) {
  let str = array.toString(); // Перетворюємо масив в рядок і видаляемо пробіли та виводимо його на консоль
  let repeatedStr = str + str; // Повторюємо рядок двічі
  console.log(repeatedStr); // Виводимо повторений рядок на консоль
}
...
function concatenateStrings(array, subString) {
  let str = array.toString(); // Використовуємо метод toString() для перетворення масиву в рядок
  let containsSubstring = str.includes(subString); //Перевіряємо чи містить рядок підрядок
  console.log(containsSubstring); // Виводимо результат
}
...
function addTenToEach(array) {
  let resultArray = new Array(array.length); // Ініціалізуємо масив через конструктор з такою же кількістю елементів як в вхідному масиві для зберігання результату
  for (let i in array) {
    // Перебираємо масив за допомогою циклу for-in
    resultArray[i] = array[i] + 10; // Додаємо до поточного елемента 10 та додаємо результат в новий масив
  }
  console.log(resultArray); // Виводимо результат
}
...
function swapMinMax(array) {
  ...
  let maxIndex = 0;
  let minIndex = 0; // Ініціалізуємо дві змінні (maxIndex і minIndex) з нульового значення. Ці змінні будуть використовуватися для зберігання індексів максимального та мінімального значень в масиві.
  for (let i = 0; i < array.length; i++) {
  ...
  [array[minIndex], array[maxIndex]] = [array[maxIndex], array[minIndex]]; // Застосовуємо деструктуризацію масиву для обміну місцями мінімального та максимального значення в масиві.
...
function getEvenNumbers(arr) {
  ...
  return evenNumbers; // Повертаємо масив результату
}
...
function multiplyByIndex(arr) {
  let multipliedArray = []; // Створюємо пустий масив для зберігання результату
  for (let i = 0; i < arr.length; i++) {
    // Проходимо через кожен елемент вхідного масиву
    multipliedArray.push(arr[i] * i); // Помножуємо число на його індекс і додаємо до масиву результату
  }
  return multipliedArray; // Повертаємо масив результату
}
...
function replaceNumbers(arr) {
  let replacedArray = []; // Створюємо пустий масив для зберігання результату
  for (let i = 0; i < arr.length; i++) {
    // Проходимо через кожен елемент вхідного масиву
    if (arr[i] > 10) {
      // Перевіряємо, чи число більше за 10
      replacedArray.push("Greater than 10"); // Якщо число більше за 10, додаємо рядок "Greater than 10" до масиву результату
    } else {
      replacedArray.push("Less than or equal to 10"); // Якщо число менше або дорівнює 10, додаємо рядок "Less than or equal to 10" до масиву результату
    }
  }
  return replacedArray; // Повертаємо масив результату
}
